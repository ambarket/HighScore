...
public class Population {
	...
	public Population(GAParams params) {
		this.orgLength = params.organismLength;
		this.popSize = params.popSize;
		this.mutProb = params.mutationProb;
		population = new Organism[popSize];
		for (int i = 0; i < popSize; i++) {
			population[i] = new Organism(orgLength, params.mutationProb);
	...
	public void moveOrganismToSortedPosition(int indexToSort) {
		...
		// TODO: Check my logic on this
		// Move the child right while its fitness is less than it's right neighbor
		while ((i < popSize-1) && (population[i].fitness < population[i + 1].fitness)) {
			tmp = population[i];
			population[i] = population[i + 1];
			population[i + 1] = tmp;
			i++;
	...
	{//select numCutPoints randomly in the range [0..orgLength-1]
	 //and store their locations in the cutPoints array
	  ...
	  for (int t = 0; (t < orgLength) && (m < numCutPoints); t++){
	    if (((orgLength - t) * Simulation.mersenneTwister.nextdouble(true, true)) < (numCutPoints - m)){
	...
	{//selects a member of the population using proportional selection scheme.
		 //If totalFitness is zero then an organism is selected at random.
		  ...
		  if (totalFitness == 0){
		    ...
		    toss = Simulation.mersenneTwister.nextDouble(true, true) * totalFitness;
		    while (sum < toss){
		      i++;
	...
	public void replaceOrganismAtIndex(int index, Organism replacement)
	{
		if (population[index].fitness < replacement.fitness) {
			population[index] = replacement;
			moveOrganismToSortedPosition(index);
		}
	}//select
	...
	{// multipoint crossover operator
	 ...
	 // the last cut point up to and including the next cut point.
	  ...
	  //now take care of the last segments, if any
	     if (child1){
	      for (int j=current; j < parent1.chromosome.length; j++){
		offspring1.chromosome[j] = parent1.chromosome[j];
		offspring2.chromosome[j] = parent2.chromosome[j];
	      }//for
	    }//if
